...
export default function ProfilePage() {
  ...
  return (
    <section className="mx-auto max-w-4xl">
      ...
      <div className="dashboard-theme-surface rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <div className="dashboard-theme-divider flex flex-col gap-6 border-b border-slate-100 pb-8 sm:flex-row sm:items-center">
          ...
          <div><h2 className="font-extrabold dark:text-white">Profile photo</h2><p className="mt-1 text-sm text-slate-500">PNG or JPEG, up to the server upload limit.</p><label className="dashboard-theme-control mt-4 inline-flex min-h-10 cursor-pointer items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-bold hover:bg-slate-50"><input type="file" accept="image/png,image/jpeg" onChange={upload} className="sr-only" disabled={uploading} />{uploading ? <Loader2 className="animate-spin" size={17} /> : <Camera size={17} />}Upload avatar</label></div>
        </div>
        <form onSubmit={save} className="mt-8 space-y-5">
          <label className="block text-sm font-bold">Full name<input value={fullName} onChange={(event) => setFullName(event.target.value)} required maxLength={100} className="dashboard-theme-control mt-2 block min-h-11 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 outline-none focus:border-cyber-500 focus:ring-4 focus:ring-cyber-500/10" /></label>
          <div className="grid gap-4 sm:grid-cols-2"><label className="block text-sm font-bold">Email<input value={user?.email || ""} disabled className="dashboard-theme-card mt-2 block min-h-11 w-full rounded-xl border border-slate-200 bg-slate-100 px-4 text-slate-500" /></label><label className="block text-sm font-bold">Username<input value={user?.username || ""} disabled className="dashboard-theme-card mt-2 block min-h-11 w-full rounded-xl border border-slate-200 bg-slate-100 px-4 text-slate-500" /></label></div>
