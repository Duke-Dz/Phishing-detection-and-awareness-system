...
export default function ProfileMenu({ open, onClose, user, avatarSrc, onLogout }) {
  ...
  return (
    <div ref={ref} className="dashboard-theme-popover absolute right-0 top-12 z-[70] w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl" role="menu">
      <div className="dashboard-theme-divider border-b border-slate-100 p-4">
      ...
      <div className="p-2">{items.map(([label, Icon, path]) => <button key={path} type="button" onClick={() => go(path)} className="dashboard-theme-hover flex min-h-10 w-full items-center gap-3 rounded-xl px-3 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:text-slate-300" role="menuitem"><Icon size={17} />{label}</button>)}
