  ...
  return (
    <section>
      ...
      {loading ? <div className="h-40 animate-pulse rounded-3xl bg-slate-100 dark:bg-slate-800" />
        ...
        : <div className={`grid gap-4 ${compact ? "" : "md:grid-cols-2 xl:grid-cols-3"}`}>{visible.map((lesson, index) => <article key={lesson.content_id} className={`flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 ${index === 0 && !compact ? "md:col-span-2 xl:col-span-1" : ""}`}>
