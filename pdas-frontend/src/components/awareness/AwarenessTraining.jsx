...
export default function AwarenessTraining({ lessons = [], loading, compact = false }) {
  ...
  return (
    <section>
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row"><div className="flex gap-2 overflow-x-auto">{categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className={`min-h-10 shrink-0 rounded-xl px-3 text-xs font-bold capitalize ${category === item ? "bg-violet-600 text-white" : "bg-white text-slate-500 dark:bg-slate-900"}`}>{item}</button>)}</div><select value={difficulty} onChange={(event) => setDifficulty(event.target.value)} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-bold capitalize dark:border-slate-700 dark:bg-slate-900">{difficulties.map((item) => <option key={item}>{item}</option>)}</select></div></>}
      {loading ? <div className="h-40 animate-pulse rounded-3xl bg-slate-100 dark:bg-slate-800" />
        : !visible.length ? <div className="rounded-3xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-900"><EmptyState icon={BookOpen} title="No training content" description="Published lessons will appear here." /></div>
        : <div className={`grid gap-4 ${compact ? "" : "md:grid-cols-2 xl:grid-cols-3"}`}>{visible.map((lesson, index) => <article key={lesson.content_id} className={`rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 ${index === 0 && !compact ? "md:col-span-2 xl:col-span-1" : ""}`}>
          ...
          <div className="mt-5 flex items-center justify-between"><p className="flex items-center gap-2 text-xs font-bold capitalize text-slate-400"><Clock size={14} />{lesson.duration_minutes || 5} min · {lesson.difficulty}</p><button type="button" onClick={() => setSelected(lesson)} className="flex min-h-10 items-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-bold text-white hover:bg-violet-700"><Play size={15} />Open</button></div>
        </article>)}</div>}
