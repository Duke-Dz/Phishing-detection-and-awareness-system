...
export default function EmailFileGuide() {
  ...
  return (
    <details className="group rounded-lg border border-cyber-200 bg-cyber-50/45 open:bg-white dark:border-slate-700 dark:bg-slate-800/70 dark:open:bg-slate-800">
      ...
      <div className="border-t border-cyber-100 p-4 dark:border-slate-700">
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <p className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">Your device</p>
            <div className="inline-flex h-9 rounded-lg border border-slate-300 bg-slate-100 p-0.5 dark:border-slate-600 dark:bg-slate-900">
          ...
          <label className="w-40 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Email app<select value={provider} onChange={(event) => setProvider(event.target.value)} className="mt-1.5 block h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-xs font-bold normal-case tracking-normal text-slate-800 outline-none transition hover:border-slate-400 focus:border-cyber-500 focus:ring-2 focus:ring-cyber-500/15 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100">{options.map((option) => <option key={option}>{option}</option>)}</select></label>
        </div>

        <ol className="mt-3 grid gap-2 sm:grid-cols-3">
          {guides[device][provider].map((step, index) => <li key={step} className="flex gap-2 rounded-lg bg-slate-50 p-2.5 text-xs leading-5 text-slate-700 dark:bg-slate-900/70 dark:text-slate-300"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white font-bold text-cyber-700 ring-1 ring-slate-200 dark:bg-slate-800 dark:text-cyber-300 dark:ring-slate-600">{index + 1}</span><span>{step}</span></li>)}
        </ol>
