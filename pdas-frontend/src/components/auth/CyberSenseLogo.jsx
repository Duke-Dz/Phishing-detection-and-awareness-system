  ...
  return (
    <div className="flex items-center justify-center gap-3">
      ...
      <div className="relative w-[64px] h-[64px] sm:w-[72px] sm:h-[72px]">
        <svg viewBox="0 0 260 260" width="100%" height="100%" className="overflow-visible" aria-hidden="true">
          <defs>
            {/* Premium Gradients */}
            ...
            <linearGradient id="shieldGradInner" x1="100%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" stopColor="#06b6d4" />
              <stop offset="100%" stopColor="#3b82f6" />
            </linearGradient>

            {/* Premium Shadows & Glows */}
            <filter id="premiumDropShadow" x="-20%" y="-20%" width="140%" height="140%">
              <feDropShadow dx="0" dy="6" stdDeviation="8" floodColor="#3b82f6" floodOpacity="0.25" />
            </filter>
          ...
          <circle cx={CENTER} cy={CENTER} r="50" fill="#3b82f6" opacity="0.08" filter="url(#neonGlow)" />
          ...
          <g transform={`rotate(${rotation}, ${CENTER}, ${CENTER})`}>
            {orbitItems.map((item) => {
              ...
              return (
                <g key={item.id} transform={`translate(${pos.x}, ${pos.y}) rotate(${-rotation})`}>
                  {/* Glassmorphism Icon Background */}
                  <circle r={ICON_R} fill="#ffffff" filter="url(#premiumDropShadow)" />
                  <circle r={ICON_R} fill={item.bg} opacity="0.9" />
                  <circle r={ICON_R} fill="none" stroke={item.color} strokeWidth="1.5" opacity="0.4" />
                  
                  {/* Center the Lucide-style SVG */}
                  <foreignObject x={-ICON_R} y={-ICON_R} width={ICON_R * 2} height={ICON_R * 2}>
                    <div className="flex h-full w-full items-center justify-center" style={{ color: item.color }}>
                      {item.icon}
                    </div>
                  </foreignObject>
                </g>
      ...
      <div className="flex flex-col justify-center">
        <span className="text-2xl sm:text-[1.7rem] font-extrabold tracking-tight text-slate-900 leading-none" style={{ letterSpacing: "-0.04em" }}>
